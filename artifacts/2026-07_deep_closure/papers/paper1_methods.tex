...
connection estimator; and the two three-dimensional field cells at SZA
$103^\circ$ ($\tau^{*} = 1$ and $3$, 550~nm), low-power at earlier
seed budgets, pass the $10^9$-photon referee at 1024 and 512 seeds
respectively with the field estimator unchanged. These field cells
referee the voxel path on the equivalent horizontally uniform field of
the same deck, against the same reference values as the corresponding
one-dimensional rows; the SHDOM cube comparisons referee genuinely
heterogeneous fields, but at moderate solar zenith angle, so an
external deep-twilight referee on a heterogeneous scene remains open. The field path
