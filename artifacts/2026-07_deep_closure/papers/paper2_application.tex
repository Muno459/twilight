...
fixed angles in worldwide use miss by 1.6 to 4.4$^{\circ}$ RMS where
they exist at all: the most widely used one is undefined on 15 of the
42 dates, and even the $15^{\circ}$ convention is undefined on 8. The
calibrated constant inverts consistently ($f = 51$ to 57) across the
central-mean desert campaigns it is calibrated on, with one documented
outlier (Hail, whose observed dawn is far shallower than every other
desert campaign) and no latitude trend: it behaves as a single
property of human dawn detection rather than a fit artifact. We further
...
\midrule
Riyadh desert (KACST) \citep{almostafa2005} & 14.50 & $14.6 \pm 0.3$ & mean & $-0.10$ \\
Hail \citep{khalifa2018} & 14.46 & $14.01 \pm 0.32$ & mean & $+0.45$ \\
Aswan 2016, camera \citep{mawad2024} & 14.82 & $14.90 \pm 0.17$ & camera mean & $-0.08$ \\
North Sinai \citep{hassan2016} & 14.16 & 14.61 & med$+\sigma$ & $-0.45$ \\
Fayum \citep{rashed2022} & 14.41 & 14.8 / SQM 13.75--14.7 & range & in range \\
Matrouh \citep{hassan2013matrouh} & 14.32 & 14.5 & photoelec.\ & $-0.18$ \\
Kottamia \citep{hassan2014} & 14.75 & $14.66 \pm 0.2$ & mean & $+0.09$ \\
Bahariya \citep{hassan2014} & 14.63 & 14.6 & photoelec.\ & $+0.03$ \\
Wadi Al-Natrun \citep{semeida2018} & 14.00 & range 12.48--15.14 & mean$+$1SD & inside \\
Tubruq (desert horizon) \citep{hassan2015tubruq} & 14.68 & 14.66--14.7 & mean$+$2SD & $0.00$ \\
Tubruq (sea horizon) \citep{hassan2015tubruq} & 14.68 & 13.43--13.48 & mean$+$2SD & $+1.22$ \\
Assiut (agricultural) \citep{hassan2016} & 14.69 & 13.665 & med$+2\sigma$ & $+1.03$ \\
Sana'a, 2200 m \citep{sultan2004} & 15.04 & bracket [13.2, 18.8] & bracket & inside \\
Depok, SQM knee \citep{saksono2020} & 14.23 & $14.0 \pm 0.6$ & SQM knee & $+0.23$ \\
Malaysia SQM sites \citep{abdelhadi2022} & 14.48 & $14.19 \pm 0.52$ & SQM knee & $+0.29$ \\
Aswan 1909, camera \citep{miethe1909} & 14.82 & 14.25 & camera & $+0.57$ \\
\bottomrule
...
to 12.9) and both peaks of the seasonal curve. The summer relaxation
...
engine's Mecca white-twilight end lies within $0.66^{\circ}$ of the
instrumental SQM twilight-end determination of \citet{niri2012} at
Tanjung Aru, Malaysia, a comparably low-latitude site ($17.33$ versus
$17.99 \pm 0.16^{\circ}$), and within $0.33^{\circ}$ of the
classical muwaqqit convention of $17^{\circ}$. The engine's red-end value ($14.96^{\circ}$) sits at the
...
\midrule
Shafaq al-abyad (white end) & $17.33^{\circ}$ & SQM end $17.99 \pm 0.16^{\circ}$ \citep{niri2012}; muwaqqit $17^{\circ}$ \\
Shafaq al-ahmar (red end) & $14.96^{\circ}$ & literature 12 to $15^{\circ}$; \citet{sultan2004} $15^{\circ}$ \\
\bottomrule
\end{tabular}
\end{table}
...
(Table~\ref{tab:conventions}); an angle counts as defined on a date
when the Sun actually reaches that depression during the night,
computed with the same NREL SPA ephemeris as the engine. The Muslim
World League angle of $18^{\circ}$ does not exist on 15 of the 42
dates: at $52.4^{\circ}$N the Sun never reaches $18^{\circ}$ of
depression once its declination exceeds about $19.6^{\circ}$, from
mid-May to late July. Where it does exist it misses by more than four
degrees, roughly half an hour of clock time. Even the $15^{\circ}$
convention is undefined on 8 midsummer dates, and misses by
$1.59^{\circ}$ RMS on the 34 where it exists. The physics-based model
is defined on every date of the year at every latitude, and is more
accurate than the best year-round fixed angle by a factor of about 1.7
(ISNA $15^{\circ}$, $1.59^{\circ}$ against the model's $0.95^{\circ}$
RMS on their common dates); the most widely used angle ($18^{\circ}$)
misses by $4.41^{\circ}$ RMS on the 27 dates where it exists.
...
Fixed $15^{\circ}$ (ISNA) & $+1.44$ & $1.59$ & 34 of 42 \\
Fixed $18^{\circ}$ (MWL) & $+4.36$ & $4.41$ & 27 of 42 \\
\bottomrule
\end{tabular}
\end{table}
...
5 skies. When the artificial veil is swept at fixed geometry (Mecca
frame, winter solstice, the production constant, quantified as the
separation between the central-patch and spread margin crossings), the
computed kadhib lead over the sadiq compresses monotonically from
$0.50^\circ$ under a clear sky to $0.29^\circ$ at Bortle 6
...
zero excess by construction). The measured response is site-dependent:
across the shipped validation runs the fitted slope is about 8 degrees
of depression per unit excess optical depth at the agricultural site
(Assiut) and about 23 at the sea-horizon site (Tubruq), with the
saturation behavior the crossing slope predicts
(Figure~
ef{fig:aod}, left panel). A sensitivity run at the shipped
...
each bar's height is the quadrature (RSS) total and its segments show
each source's share of the variance; the Birmingham winter row is
dominated by the skyglow term, itself dominated by the street-light
duty cycle.}
\label{fig:aod}
\end{figure}
...
\bibitem[Hassan et al.(2013)]{hassan2013matrouh}
Hassan, A.H., Hassanin, N.Y., Abdel-Hadi, Y.A., Issa, I.A., 2013.
Time verification of twilight begin and end at Matrouh of Egypt.
NRIAG Journal of Astronomy and Geophysics 2, 45--53. Photoelectric
scanner (10~cm refractor, blue/visual/red filters) plus naked eye;
dawn onset $14$ to $16^\circ$ of depression.
...
\bibitem[Niri et al.(2012)]{niri2012}
Niri, M.A., Zainuddin, M.Z., Man, S., Mohd Nawawi, M.S.A., Abdul
Wahab, R., Ismail, K., Ahmad Zaki, N.H., Ab Ghani, A., Lokman,
M.A.A., 2012. Astronomical determinations for the beginning prayer
time of Isha'. Middle-East Journal of Scientific Research 12,
101--107. SQM-LE at Tanjung Aru, Sabah, Malaysia; shafaq al-abyad
disappearance at mean solar depression $17.99 \pm 0.16^\circ$.

\bibitem[Rashed et al.(2022)]{rashed2022}
Rashed, M.G., Abdel-Hadi, Y.A., El-Nawawy, M.S., et al., 2022.
Determination of the true dawn by several different ways at Fayum in
Egypt. International Journal of Mechanical Engineering and Technology
13(10), 8--24. SQM-LU-DL plus trained naked-eye group at Wadi al
Hitan; true dawn between $14$ and $14.8^\circ$ across four criteria.

\bibitem[Saksono and Fulazzaky(2020)]{saksono2020}
Saksono, T., Fulazzaky, M.A., 2020. Predicting the accurate period of
true dawn using a third-degree polynomial model. NRIAG Journal of
Astronomy and Geophysics 9, 238--244. SQM at Depok, Indonesia, 26
mornings; dawn knee at $14.0 \pm 0.6^\circ$ of depression.

\bibitem[Abdel-Hadi and Hassan(2022)]{abdelhadi2022}
Abdel-Hadi, Y.A.-F., Hassan, A.H., 2022. The effect of sun elevation
on the twilight stages in Malaysia. International Journal of Astronomy
and Astrophysics 12, 7--29. SQM at five Malaysian sites, 2007--2008;
true dawn $14.19 \pm 0.52^\circ$ (high-confidence $14.71^\circ$),
pseudo-dusk end $17.8 \pm 0.7^\circ$.
